...
def test_sql_uses_index(caplog):
    ...
    s = SqlReader("SELECT * FROM tests.data.index.is WHERE user_name='Verizon Support'", inner_reader=DiskReader, raw_path=True)
    findings = list(s)
    assert len(findings) == 2, len(findings)

    if caplog:
        has_index_log = any("Reading index from" in d for n,l,d in caplog.record_tuples)
